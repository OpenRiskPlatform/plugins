...
function relationEntityRcas(
  ...
  const names = getPropertyValues(value.properties ?? {}, personKeys)
    .filter((name) => name.toLowerCase() !== targetName.toLowerCase());

  if (!names.length) {
    const fallbackName = propertyValueText(value);
    return fallbackName ? [{ name: fallbackName, relation }] : [];
  }
...
function rcasFromPropertyValue(
  ...
  if (schema === "family") {
    return relationEntityRcas(value, targetName, ["relationship"], ["relative", "person"], fallbackRelation);
  }
  if (schema === "associate") {
    return relationEntityRcas(value, targetName, ["relationship"], ["associate", "person"], fallbackRelation);
  }

  const name = propertyValueText(value);
  return name ? [{ name, relation: fallbackRelation }] : [];
}
...
function extractRelativeCloseAssociates(
  ...
  const groups: Array<{ keys: string[]; relation: string }> = [
    { keys: ["familyPerson"], relation: "family member" },
    { keys: ["familyRelative"], relation: "relative" },
    { keys: ["associates"], relation: "associate" },
    { keys: ["associations"], relation: "association" },
  ];

  return uniqueRca(groups.flatMap(({ keys, relation }) =>
    keys.flatMap((key) =>
      (properties[key] ?? []).flatMap((value) => rcasFromPropertyValue(value, targetName, relation))
    )
  ));
}
...
function normalizePersonEntity(
  ...
  const relativeCloseAssociates = extractRelativeCloseAssociates(properties, name);
