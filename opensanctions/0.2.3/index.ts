...
const OPEN_SANCTIONS_MATCH_COST_PER_QUERY_EUR = 0.1;
const RCA_ENRICHMENT_CALL_LIMIT = 20;
...
interface OpenSanctionsEntity {
  ...
  caption?: string;
  schema: string;
  properties?: Record<string, OpenSanctionsPropertyValue[] | undefined>;
  datasets?: string[];
  referents?: string[];
  first_seen?: string;
  last_seen?: string;
...
type OpenSanctionsPropertyValue = string | OpenSanctionsEntity;
...
function propertyValueText(value: OpenSanctionsPropertyValue | undefined): string | undefined {
  if (typeof value === "string") return value;
  if (!value) return undefined;

  return value.caption?.trim() || getPropertyValues(value.properties ?? {}, ["name"])[0] || value.id;
}

function relatedPartyText(value: OpenSanctionsPropertyValue | undefined): string | undefined {
  if (!value) return undefined;
  if (typeof value === "string") {
    const text = value.trim();
    return /^Q\d+$/i.test(text) || /^NK-/i.test(text) ? undefined : text;
  }

  return value.caption?.trim() || getPropertyValues(value.properties ?? {}, ["name"])[0];
}
...
function getPropertyValues(
  ...
  return uniqueStrings(keys.flatMap((key) => properties[key] ?? []).map(propertyValueText));
}

function getRelatedPartyNames(
  properties: Record<string, OpenSanctionsPropertyValue[] | undefined>,
  keys: string[],
): string[] {
  return uniqueStrings(keys.flatMap((key) => properties[key] ?? []).map(relatedPartyText));
}

function uniqueRca(values: RelativeCloseAssociate[]): RelativeCloseAssociate[] {
  const seen = new Set<string>();
  const out: RelativeCloseAssociate[] = [];
...
function relationEntityRcas(
  value: OpenSanctionsEntity,
  targetName: string,
  targetId: string,
  relationKeys: string[],
  personKeys: string[],
  fallbackRelation: string,
): RelativeCloseAssociate[] {
  const relation = getPropertyValues(value.properties ?? {}, relationKeys)[0] || fallbackRelation;
  const names = getRelatedPartyNames(value.properties ?? {}, personKeys)
    .filter((name) =>
      name.toLowerCase() !== targetName.toLowerCase() &&
      name.toLowerCase() !== targetId.toLowerCase()
    );

  return names.map((name) => ({ name, relation }));
}

function rcasFromPropertyValue(
  value: OpenSanctionsPropertyValue,
  targetName: string,
  targetId: string,
  fallbackRelation: string,
  relationPartyKeys: string[],
): RelativeCloseAssociate[] {
  if (typeof value === "string") {
    const name = relatedPartyText(value);
    return name ? [{ name, relation: fallbackRelation }] : [];
  }

  const schema = value.schema.toLowerCase();
  if (schema === "family") {
    return relationEntityRcas(value, targetName, targetId, ["relationship"], relationPartyKeys, fallbackRelation);
  }
  if (schema === "associate") {
    return relationEntityRcas(value, targetName, targetId, ["relationship"], relationPartyKeys, fallbackRelation);
  }
...
function extractRelativeCloseAssociates(
  properties: Record<string, OpenSanctionsPropertyValue[] | undefined>,
  targetName: string,
  targetId: string,
): RelativeCloseAssociate[] {
  const groups: Array<{ keys: string[]; relation: string; relationPartyKeys: string[] }> = [
    { keys: ["familyPerson"], relation: "family member", relationPartyKeys: ["relative"] },
    { keys: ["familyRelative"], relation: "relative", relationPartyKeys: ["person"] },
    { keys: ["associates"], relation: "associate", relationPartyKeys: ["associate", "person"] },
    { keys: ["associations"], relation: "association", relationPartyKeys: ["associate", "person"] },
  ];

  return uniqueRca(groups.flatMap(({ keys, relation, relationPartyKeys }) =>
    keys.flatMap((key) =>
      (properties[key] ?? []).flatMap((value) =>
        rcasFromPropertyValue(value, targetName, targetId, relation, relationPartyKeys)
      )
    )
  ));
}
...
function normalizePersonEntity(
  ...
  const relativeCloseAssociates = extractRelativeCloseAssociates(properties, name, entity.id);
...
async function fetchNestedEntity(
  entityId: string,
  token: string,
): Promise<OpenSanctionsEntity | undefined> {
  const url = new URL(`/entities/${encodeURIComponent(entityId)}`, OPENSANCTIONS_API_URL);
  ...
  return (await response.json()) as OpenSanctionsEntity;
}

async function enrichPersonResultsWithRca(
  results: OpenSanctionsEntity[],
  token: string,
): Promise<OpenSanctionsEntity[]> {
  const enriched: OpenSanctionsEntity[] = [];
  let remainingRcaCalls = RCA_ENRICHMENT_CALL_LIMIT;

  for (const entity of results) {
    if (!isPersonSchema(entity.schema) || remainingRcaCalls <= 0) {
      enriched.push(entity);
      continue;
    }

    remainingRcaCalls -= 1;
    const nestedEntity = await fetchNestedEntity(entity.id, token);
    if (!nestedEntity) {
      enriched.push(entity);
      continue;
    }
...
export async function matchPersons(inputs: PluginInputs): Promise<DataModelEntity[]> {
  ...
  const enrichedResults = await enrichPersonResultsWithRca(results, token);

  const entities = enrichedResults.map((entity) =>
  ...
  metricSet("status", `${OPEN_SANCTIONS_MATCH_COST_PER_QUERY_EUR.toFixed(2)} EUR used`);
