...
// ---------------------------------------------------------------------------
// Run finalizer - records remaining credit in the status metric.
// ---------------------------------------------------------------------------
...
async function finalizeRun(apiKey: string, entities: DataModelEntity[]): Promise<DataModelEntity[]> {
    try {
        ...
        metricSet("status", credit.toFixed(2) + " EUR remaining");
    } catch {
        // Credit status is supplemental; a failed stat call must not fail the scan.
    }
    return entities;
}
