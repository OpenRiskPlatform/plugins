...
const REGISTRY_JURISDICTION_SUBDIVISION_LABELS: Record<string, Record<string, string>> = {
  ae: {
    az: "Abu Dhabi",
    du: "Dubai",
  },
...
function buildPerson(opts: _OR_Opts<PersonPayload>): DataModelEntity {
  ...
  _or_set(
    props,
    "jurisdiction",
    p.jurisdiction ? _tv.str(p.jurisdiction) : undefined,
  );
...
function registryJurisdictionLabel(code: RegistryJurisdictionCode): string {
  const [countryCode, subdivisionCode] = code.split("_");
  const countryLabel = REGION_DISPLAY_NAMES?.of(countryCode.toUpperCase())
    ?? countryCode.toUpperCase();

  if (!subdivisionCode) return countryLabel;

  const subdivisionLabel =
    REGISTRY_JURISDICTION_SUBDIVISION_LABELS[countryCode]?.[subdivisionCode]
...
function normalizeCompanyEntity(
  ...
  const jurisdictionLabel = jurisdictionCode
    ? registryJurisdictionLabel(jurisdictionCode)
    : undefined;
