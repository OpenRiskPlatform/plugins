...
|------|------|----------|-------------|
| `token` | secret string | **Yes** | API token for authenticated requests to OpenCorporates. |
...
| `jurisdiction_code` | registry-jurisdiction-code | No | — | OpenCorporates registry jurisdiction code, e.g. `gb`, `us_de`, `sk`. |
...
Result `jurisdiction` properties are emitted as full English names rather than raw jurisdiction codes.
